...
class AssessmentAnalytics:
    ...
    def SNKcompareTreatments(self, i, k, num_thesis, sortMeans, qSwit,
        ...
        for m in range(num_thesis-1, k, -1):
            ...
            criticalQ = self.qCriticalSNK(df, p)
            if self._debug:
    ...
    def qCriticalSNK(self, df, p):
        # rows are df, degrees of freedom
        # columsn are groups, first column is 2 and last is 20
        if df < 1:
            return None
        elif df > 120:
            return SNK_qCritical__0_05['inf'][p-2]
        elif df not in SNK_qCritical__0_05:
            return self.qCriticalSNK(df-5, p)
        if p > 1 and p < 20:
            return SNK_qCritical__0_05[df][p-2]
        return None
